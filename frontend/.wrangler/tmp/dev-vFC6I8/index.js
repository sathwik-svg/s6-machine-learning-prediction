  ...
  async fetch(request) {
    ...
    if (url.pathname === "/predict" && request.method === "POST") {
      try {
        ...
        if (age < 18 || income <= 0 || years < 0 || satisfaction < 1 || satisfaction > 5 || balance < 1 || balance > 5) {
          return Response.json(
            { error: "Invalid prediction input" },
            { status: 400 }
          );
        }
        const score = -2.2 + overtime * 1.4 + (30 - Math.min(age, 30)) * 0.025 + (3 - satisfaction) * 0.65 + (3 - balance) * 0.55 - years * 0.08 - income / 1e4 * 0.25;
        const probability = 1 / (1 + Math.exp(-score));
        const riskPercentage = Number((probability * 100).toFixed(2));
        return Response.json({
          prediction: probability >= 0.5 ? "High Attrition Risk" : "Low Attrition Risk",
...
// .wrangler/tmp/bundle-IBCsX9/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-IBCsX9/middleware-loader.entry.ts
